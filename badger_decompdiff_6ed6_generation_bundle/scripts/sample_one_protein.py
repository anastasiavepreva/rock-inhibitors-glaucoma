...
import utils.prior as utils_prior  # noqa: E402
import utils.transforms as trans  # noqa: E402
from models.classifier import Classifier  # noqa: E402
from models.decompdiff import DecompScorePosNet3D  # noqa: E402
from scripts.sample_diffusion_decomp import sample_diffusion_ligand_decomp  # noqa: E402
from utils import data as utils_data  # noqa: E402
from utils.data import PDBProtein, ProteinLigandData, torchify_dict  # noqa: E402
...
if __name__ == "__main__":
    ...
    parser.add_argument("--classifier_ckpt_dir", type=str, default=None)
    ...
    ckpt_path = resolve_repo_path(
        args.ckpt_path if args.ckpt_path else config.model.checkpoint
    )
    ckpt = torch.load(ckpt_path, map_location=args.device, weights_only=False)
    if "train_config" in config.model:
        ckpt["config"] = misc.load_config(
            resolve_repo_path(config.model["train_config"])
        )
    ...
    # Default to subpocket priors for novel single-pocket sampling.
    prior_mode = args.prior_mode or "subpocket"
    init_transform_list = [
    ...
    if prior_mode == "ref_prior":
        ...
        beta_prior_dir = resolve_repo_path(args.beta_prior_path)
        beta_pkl = os.path.join(beta_prior_dir, f"{pocket_id}.pkl")
    ...
    # Load classifier
    # ---------------------------------------------------------------------
    classifier_ckpt_dir = resolve_repo_path(
        args.classifier_ckpt_dir if args.classifier_ckpt_dir else config.sample.ckpt
    )
    clf_ckpt = torch.load(
        os.path.join(classifier_ckpt_dir, "ckpt.pt"),
    ...
    results = sample_diffusion_ligand_decomp(
        ...
        natoms_config=resolve_repo_path(args.natom_models_path),
